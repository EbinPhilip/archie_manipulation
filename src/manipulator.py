...
prePickPose.transform.translation.x = -0.18
...
holdPoseJointValuesDeg = [90, -20, 63, 0, -42, 0]
placePoseJointValuesDeg = [38, 32, 24, 43, -64, -21]

def toRadians(positionList: list) -> list:
    ret = []
    value: float
    for value in positionList:
        ret.append(radians(value))
    return ret
...
class Manipulator:
    ...
    def closeGripper(self, position = 0.05, effort = 0.270):
        goal = GCGoal()
        goal.command.position = position
        goal.command.max_effort = effort
        self._executeGripperAction(goal)
    ...
    def place(self, objectName:str):
        self.goToPlacePose()
        self.openGripper()
        self.moveGroup.detach_object(objectName)
    
    def goToHoldingPose(self):
        jointPositions = toRadians(holdPoseJointValuesDeg)
        self.moveGroup.go(jointPositions)
    
    def goToPlacePose(self):
        jointPositions = toRadians(placePoseJointValuesDeg)
        self.moveGroup.go(jointPositions)
